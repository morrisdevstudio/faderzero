...
Objectif: garder le developpement local confortable sur ce PC, tout en deployant automatiquement une version "reelle" de la PWA sur `192.168.1.71`, installable depuis Chrome Android.

## Architecture cible

- Ce PC sert a coder et lancer `npm run dev`.
- Le serveur `192.168.1.71` heberge la version de test reel.
- Un conteneur `tailscale/tailscale` tourne sur `192.168.1.71`.
...
- il ne doit pas pointer vers `http://192.168.1.71:54321`, sinon Chrome Android bloquera les requetes

Le script `npm run build:deploy` utilise automatiquement ce mode `deploy`.

## 2. Etat actuel du serveur `192.168.1.71`

Le setup actuellement en place est le suivant:

- SSH via `docker-yapi@192.168.1.71`
- Docker disponible pour `docker-yapi`
- conteneur `faderzero-caddy`
- conteneur `faderzero-tailscale`
- release active dans `/home/docker-yapi/appGroup/faderzero-pwa/current`
- URL privee finale: `https://faderzero-server.tailfba668.ts.net/`

Si tu veux recreer ou verifier l'arborescence cible:

```bash
mkdir -p /home/docker-yapi/appGroup/faderzero-pwa/releases
```

Les fichiers utilises cote serveur sont:

- [`pwa/deploy/Caddyfile`](/D:/App%20en%20dev/FaderZeroPWA/pwa/deploy/Caddyfile:1)
- [`pwa/deploy/docker-compose.remote.yml`](/D:/App%20en%20dev/FaderZeroPWA/pwa/deploy/docker-compose.remote.yml:1)
...
```bash
https://faderzero-server.tailfba668.ts.net/
```
...
- `npm run deploy:test` : build `deploy` puis copie `pwa/dist` sur `192.168.1.71`
...
- archive `dist`
- envoie l'archive en SSH sur `192.168.1.71`
...
```powershell
$env:FADERZERO_DEPLOY_HOST = "192.168.1.71"
$env:FADERZERO_DEPLOY_USER = "docker-yapi"
$env:FADERZERO_DEPLOY_BASE_DIR = "/home/docker-yapi/appGroup/faderzero-pwa"
```
...
1. Ouvrir [https://faderzero-server.tailfba668.ts.net/](https://faderzero-server.tailfba668.ts.net/) dans Chrome Android
...
- Le deploiement automatique pousse uniquement les fichiers statiques de `pwa/dist`, pas tout le depot.
