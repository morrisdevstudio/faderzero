...
`npm run icons:capture` lance les scénarios statiques prêts via Playwright et son `webServer` Vite. Chaque SVG visible reçoit uniquement dans le DOM de test `data-icon-audit-instance=<scenarioId>-<ordinal>` : cette convention est déterministe par scénario, n’est ni compilée ni envoyée en production, et distingue les instances d’un même composant. Le runner rapproche ensuite l’empreinte de forme avec l’inventaire, conserve les décisions manuelles et écrit `capture-report.json`.
