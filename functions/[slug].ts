...
type PublishedRow = { display_name: string; slug: string; published_revision: number; published_snapshot: unknown };
...
export const onRequestGet = async (context: PagesContext): Promise<Response> => {
  ...
  const row = await loadPublishedEpk(context.env, slug);
  ...
  const model = withPublicMedia(row.published_snapshot, context.env.EPK_PUBLIC_MEDIA_ORIGIN);
  const metadata = socialMetadata(model, requestUrl);
  const payload = safeJson(model);
  const injected = html.replace('</head>', `${metadata}<script>window.__FZ_EPK_MODEL__=${payload};</script></head>`);
  return new Response(injected, { status: 200, headers: { ...publicHeaders(row.published_revision), 'content-type': 'text/html; charset=UTF-8' } });
};

export const onRequestHead = async (context: PagesContext): Promise<Response> => {
  const slug = context.params.slug?.toLowerCase();
  const row = slug ? await loadPublishedEpk(context.env, slug) : null;
  return row ? new Response(null, { status: 200, headers: publicHeaders(row.published_revision) }) : redirectToLanding();
};

async function loadPublishedEpk(env: PagesContext['env'], slug: string): Promise<PublishedRow | null> {
  const endpoint = new URL(`${env.SUPABASE_URL}/rest/v1/epks`);
  endpoint.searchParams.set('select', 'display_name,slug,published_revision,published_snapshot');
...
function publicHeaders(revision: number): Record<string, string> {
  return { 'cache-control': 'no-store', 'x-fz-epk-revision': String(revision), 'access-control-allow-origin': APP_ORIGIN, vary: 'Origin' };
}
