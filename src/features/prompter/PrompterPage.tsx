...
function BackIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="m15 18-6-6 6-6" /></svg>;
}
...
export function PrompterPage() {
  ...
  const [sourceOpen, setSourceOpen] = useState(!params.get('setlistId') && !params.get('songId'));
  ...
  function chooseSource(nextSetlistId?: string, nextSongId?: string) { const nextParams = new URLSearchParams(); if (nextSetlistId) nextParams.set('setlistId', nextSetlistId); if (nextSongId) nextParams.set('songId', nextSongId); setParams(nextParams); setSelectedSongId(nextSongId ?? null); setSourceOpen(false); }
  ...
  return <div className="flex h-[100dvh] flex-col overflow-hidden overscroll-none bg-[var(--fz-bg)] text-white">
    ...
    <main className="mx-auto flex min-h-0 w-full max-w-5xl flex-1 flex-col overflow-hidden bg-black px-4 sm:px-6">{selectedSong ? <><section className="flex h-16 shrink-0 touch-none select-none flex-col justify-center border-b border-white/8 text-center"><button type="button" onClick={() => setSourceOpen(true)} className="block w-full truncate text-lg font-black sm:text-xl">{selectedSong.title || 'Sans titre'}</button><div className="mt-1 flex items-center justify-center gap-2 truncate text-[0.68rem] font-bold text-white/60"><span>{selectedSong.bpm ? `${selectedSong.bpm} BPM` : '— BPM'}</span><span aria-hidden="true">·</span><span>{formatDuration(selectedSong.durationSeconds)}</span><span aria-hidden="true">·</span><span>{selectedSong.key || '— Ton'}</span></div></section><div ref={contentRef} onPointerDown={() => { paused.current = true; }} onPointerUp={() => { window.setTimeout(() => { paused.current = false; }, 500); }} className="min-h-0 flex-1 overscroll-contain overflow-y-auto px-1 py-8 sm:px-12 sm:py-12"><Lyrics text={selectedSong.lyrics} scale={preferences.scale} /><p className="pb-8 pt-20 text-center text-xs italic text-white/35">— Fin du morceau —</p></div><div className="pointer-events-none fixed inset-x-0 bottom-[max(4rem,env(safe-area-inset-bottom))] z-30"><div className="mx-auto grid max-w-5xl grid-cols-2 gap-3 px-4 sm:px-6"><button type="button" disabled={!previousSong} onClick={() => moveSong(previousSong)} className={`${navigationButtonClass} justify-start text-left`}><span>‹ Précédent<br /><span className="text-white">{previousSong?.title ?? 'Début'}</span></span></button><button type="button" disabled={!nextSong} onClick={() => moveSong(nextSong)} className={`${navigationButtonClass} justify-end text-right`}><span>Suivant ›<br /><span className="text-white">{nextSong?.title ?? 'Fin'}</span></span></button></div></div></> : <div className="flex flex-1 items-center justify-center text-center"><div><p className="text-lg font-black">Aucun morceau disponible</p><p className="mt-2 text-sm text-white/55">Ajoute des chansons ou choisis une autre source.</p><button type="button" onClick={() => setSourceOpen(true)} className="mt-5 rounded-xl bg-white px-4 py-3 text-sm font-black text-[#111319]">Changer de sélection</button></div></div>}</main>
    {sourceOpen ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 sm:items-center sm:p-6"><section role="dialog" aria-modal="true" className="max-h-[88dvh] w-full max-w-xl overflow-y-auto rounded-t-2xl border border-white/10 bg-[#111318] p-5 sm:rounded-2xl"><div className="flex items-start justify-between"><div><h2 className="text-xl font-black">Que joue-t-on ?</h2><p className="mt-1 text-sm text-white/55">Choisis une setlist ou le répertoire complet.</p></div><button type="button" onClick={() => setSourceOpen(false)} aria-label="Fermer" className="fz-dialog-close">×</button></div><div className="mt-5 space-y-2"><button type="button" onClick={() => chooseSource()} className="w-full rounded-xl border border-emerald-300/35 bg-emerald-300/10 p-4 text-left"><p className="font-black">Toutes les chansons</p><p className="mt-1 text-xs text-white/55">{songs.length} morceau{songs.length > 1 ? 'x' : ''}, classés par titre</p></button><p className="px-1 pt-4 text-xs font-black uppercase tracking-[0.16em] text-white/40">Setlists</p>{setlists.length ? setlists.map((item) => <button key={item.id} type="button" onClick={() => chooseSource(item.id)} className="w-full rounded-xl border border-white/10 bg-white/5 p-4 text-left"><p className="font-black">{item.name}</p><p className="mt-1 text-xs text-white/55">Ouvrir cette liste de concert</p></button>) : <p className="rounded-xl border border-dashed border-white/12 p-4 text-sm text-white/55">Aucune setlist disponible.</p>}</div></section></div> : null}
  </div>;
}
