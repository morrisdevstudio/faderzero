...
const accentSwatches = [['#ff3a63', 'Rose FaderZero', 'bg-[#ff3a63]'], ['#f97316', 'Orange', 'bg-orange-500'], ['#facc15', 'Jaune', 'bg-yellow-400'], ['#4ade80', 'Vert', 'bg-green-400'], ['#2dd4bf', 'Turquoise', 'bg-teal-400'], ['#38bdf8', 'Bleu ciel', 'bg-sky-400'], ['#818cf8', 'Indigo', 'bg-indigo-400'], ['#c084fc', 'Violet', 'bg-purple-400'], ['#ffffff', 'Blanc', 'bg-white'], ['#a8a8a8', 'Gris', 'bg-[#a8a8a8]']] as const;
...
export function EpkEditorFields(props: Props) {
  ...
  return <><div className="space-y-5">
    ...
    <Section title="URL et couleur d’accent"><div><p className="fz-field-label">URL</p><div className="flex min-h-12 items-center gap-2"><p className="min-w-0 flex-1 truncate text-base text-white">faderzero.com/{epk.slug}</p><EditIconButton label="Modifier l’URL" usageId="epk.url.edit" disabled={saving} onClick={() => setUrlDialogOpen(true)} /></div></div><fieldset><legend className="fz-field-label">Couleur d’accent</legend><div className="mt-2 grid grid-cols-5 gap-2">{accentSwatches.map(([value, label, className]) => <button key={value} type="button" aria-label={label} aria-pressed={(epk.accentColor ?? '#ff3a63') === value} onClick={() => onChange({ ...epk, accentColor: value })} className={`h-11 w-11 rounded-full border-2 ${className} ${(epk.accentColor ?? '#ff3a63') === value ? 'border-white ring-2 ring-white/35' : value === '#ffffff' ? 'border-white/25' : 'border-transparent'}`} />)}</div></fieldset></Section>
