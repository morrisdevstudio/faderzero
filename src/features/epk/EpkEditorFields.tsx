...
import type { AvailableEpkTrack, EpkContact, EpkDocument, EpkLink, EpkPhoto, EpkRecord, EpkTrack, EpkVideo, EpkVideoType } from './epk';
import { DEFAULT_EPK_DOCUMENT_ICON, DEFAULT_EPK_FACT_ICON, DEFAULT_EPK_SECTION_ORDER, normalizedEpkFactIcon, type EpkDocumentIcon, type EpkFact, type EpkFactIcon, type EpkSectionId } from './epkPresentation';
...
const accentSwatches = [['#ff3a63', 'Rose FaderZero', 'bg-[#ff3a63]'], ['#f97316', 'Orange', 'bg-orange-500'], ['#facc15', 'Jaune', 'bg-yellow-400'], ['#4ade80', 'Vert', 'bg-green-400'], ['#38bdf8', 'Bleu ciel', 'bg-sky-400'], ['#818cf8', 'Indigo', 'bg-indigo-400'], ['#c084fc', 'Violet', 'bg-purple-400'], ['#ffffff', 'Blanc', 'bg-white'], ['#a8a8a8', 'Gris', 'bg-[#a8a8a8]'], ['#000000', 'Noir', 'bg-black']] as const;
...
const factIcons: Array<{ value: EpkFactIcon; label: string }> = [
  { value: 'location', label: 'Localisation' },
  { value: 'globe-2', label: 'Monde' },
  { value: 'flag', label: 'Drapeau' },
  { value: 'building-2', label: 'Lieu' },
  { value: 'guitar', label: 'Guitare' },
  { value: 'drum', label: 'Batterie' },
  { value: 'music-note', label: 'Musique' },
  { value: 'mic-vocal', label: 'Voix' },
  { value: 'users', label: 'Groupe' },
  { value: 'user-round', label: 'Personne' },
  { value: 'calendar', label: 'Calendrier' },
  { value: 'clock', label: 'Horloge' },
  { value: 'star', label: 'Favori' },
  { value: 'heart', label: 'Cœur' },
  { value: 'radio', label: 'Radio' },
  { value: 'disc-3', label: 'Disque' },
  { value: 'zap', label: 'Énergie' },
  { value: 'flame', label: 'Flamme' },
  { value: 'languages', label: 'Langues' },
  { value: 'sparkles', label: 'Éclat' },
];
const documentIcons: Array<{ value: EpkDocumentIcon; label: string }> = [
  { value: 'clipboard-list', label: 'Liste' },
  { value: 'list-checks', label: 'Checklist' },
  { value: 'wrench', label: 'Technique' },
  { value: 'cable', label: 'Câblage' },
  { value: 'mic-vocal', label: 'Voix' },
  { value: 'speaker', label: 'Sono' },
  { value: 'audio-lines', label: 'Audio' },
  { value: 'images', label: 'Photos' },
  { value: 'briefcase-business', label: 'Pro' },
  { value: 'file-music', label: 'Partition' },
  { value: 'file-archive', label: 'Archive' },
  { value: 'file-text', label: 'Document' },
];

type Props = {
  epk: EpkRecord; onChange: (next: EpkRecord) => void; onPublish: () => void; onUnpublish: () => void; onViewPublished: () => void; saving: boolean;
  ...
  heroPreviewUrl?: string; onUploadHero: (file: File) => void; onRemoveHero: () => void; onUploadPhoto: (file: File) => void; onRemovePhoto: (item: EpkPhoto) => void; onUploadDocument: (file: File, title: string, description: string, icon: EpkDocumentIcon) => void; onUpdateDocument: (item: EpkDocument, title: string, description: string, icon: EpkDocumentIcon, file?: File) => void; onRemoveDocument: (item: EpkDocument) => void;
...
export function EpkEditorFields(props: Props) {
  ...
  const [addDialog, setAddDialog] = useState<'fact' | 'track' | 'video' | 'document' | null>(null); const [contactDialogOpen, setContactDialogOpen] = useState(false); const [editingContact, setEditingContact] = useState<EpkContact | null>(null); const [editingDocument, setEditingDocument] = useState<EpkDocument | null>(null); const [editingFact, setEditingFact] = useState<EpkFact | null>(null); const [urlDialogOpen, setUrlDialogOpen] = useState(false);
  ...
  return <><div className="space-y-5">
    <div className="space-y-2"><Button variant="primary" fullWidth loading={saving} onClick={props.onPublish}>{saving ? 'Publication…' : epk.status === 'PUBLISHED' ? 'Mettre à jour la page publique' : 'Publier la page'}</Button>{epk.status === 'PUBLISHED' ? <><Button variant="secondary" fullWidth disabled={saving} onClick={props.onViewPublished}>Voir la page publiée</Button><Button variant="danger" fullWidth disabled={saving} onClick={props.onUnpublish}>Retirer la page publique</Button></> : null}</div>
    <Section title="URL et apparence"><div><p className="fz-field-label">URL</p><div className="flex min-h-12 items-center gap-2"><p className="min-w-0 flex-1 truncate text-base text-white">faderzero.com/{epk.slug}</p><EditIconButton label="Modifier l’URL" usageId="epk.url.edit" disabled={saving} onClick={() => setUrlDialogOpen(true)} /></div></div><div><FieldLabel htmlFor="epk-theme">Thème</FieldLabel><SelectField id="epk-theme" value={epk.theme === 'press-ivory' ? 'press-ivory' : 'stage-dark'} disabled={saving} onChange={(event) => onChange({ ...epk, theme: event.target.value === 'press-ivory' ? 'press-ivory' : 'stage-dark' })}><option value="stage-dark">Sombre</option><option value="press-ivory">Clair — Ivoire</option></SelectField></div><fieldset><legend className="fz-field-label">Couleur d’accent</legend><div className="mt-2 grid grid-cols-5 gap-2">{accentSwatches.map(([value, label, className]) => <button key={value} type="button" aria-label={label} aria-pressed={(epk.accentColor ?? '#ff3a63') === value} onClick={() => onChange({ ...epk, accentColor: value })} className={`h-11 w-11 rounded-full border-2 ${className} ${(epk.accentColor ?? '#ff3a63') === value ? 'border-white ring-2 ring-white/35' : value === '#ffffff' || value === '#000000' ? 'border-white/25' : 'border-transparent'}`} />)}</div></fieldset></Section>
    ...
    <Section title="Biographie"><Field id="bio-title" label="Titre" value={epk.editorial.bioTitle} onChange={(bioTitle) => onChange({ ...epk, editorial: { ...epk.editorial, bioTitle } })} /><div><FieldLabel htmlFor="epk-bio-text">Texte de biographie</FieldLabel><TextArea id="epk-bio-text" rows={7} value={epk.fullBio ?? ''} onChange={(event) => onChange({ ...epk, fullBio: event.target.value })} /></div><Collection title="En bref">{epk.editorial.facts.map((fact) => <Card key={fact.id} onRemove={() => updateFacts(epk.editorial.facts.filter((item) => item.id !== fact.id))} onEdit={() => setEditingFact(fact)} disabled={saving}><div className="flex items-center gap-2"><FzIcon name={normalizedEpkFactIcon(fact.icon)} usageId={`epk.facts.card.${fact.id}`} size="lg" /><p className="min-w-0 truncate font-semibold">{fact.title || fact.value}</p></div>{fact.title ? <p className="truncate text-sm text-white/65">{fact.value}</p> : null}</Card>)}<Button variant="primary" fullWidth leadingIcon={<FzIcon name="add" usageId="epk.facts.add" />} disabled={saving} onClick={() => { setEditingFact(null); setAddDialog('fact'); }}>Ajouter un élément</Button></Collection></Section>
  ...
  </div>{urlDialogOpen ? <EpkUrlDialog slug={epk.slug} saving={saving} onClose={() => setUrlDialogOpen(false)} onSubmit={(slug) => onChange({ ...epk, slug })} /> : null}{addDialog === 'fact' || editingFact ? <FactFormDialog saving={saving} initialFact={editingFact} onClose={() => { setAddDialog(null); setEditingFact(null); }} onAdd={(fact) => { if (editingFact) updateFacts(epk.editorial.facts.map((item) => (item.id === editingFact.id ? fact : item))); else updateFacts([...epk.editorial.facts, fact]); }} /> : null}{addDialog === 'track' ? <TrackFormDialog saving={saving} tracks={props.availableTracks.filter((track) => track.isSynced && !props.tracks.some((item) => item.songAssetId === track.id))} onClose={() => setAddDialog(null)} onAdd={props.onAddTrack} /> : null}{addDialog === 'video' ? <VideoFormDialog saving={saving} onClose={() => setAddDialog(null)} onAdd={props.onAddVideo} /> : null}{addDialog === 'document' || editingDocument ? <DocumentFormDialog saving={saving} initialDocument={editingDocument} onClose={() => { setAddDialog(null); setEditingDocument(null); }} onUpload={props.onUploadDocument} onUpdate={(title, description, icon, file) => { if (editingDocument) props.onUpdateDocument(editingDocument, title, description, icon, file); }} /> : null}{contactDialogOpen || editingContact ? <ContactFormDialog saving={saving} initialContact={editingContact} onClose={() => { setContactDialogOpen(false); setEditingContact(null); }} onAdd={(name, role, email, phone) => { if (editingContact && props.onUpdateContact) { props.onUpdateContact(editingContact.id, name, role, email, phone); } else { props.onAddContact(name, role, email, phone); } }} /> : null}</>;
}
...
export function FactFormDialog({ saving, onClose, onAdd, initialFact }: { saving: boolean; onClose: () => void; onAdd: (fact: EpkFact) => void; initialFact?: EpkFact | null }) {
  const isEditing = Boolean(initialFact);
  const [title, setTitle] = useState(initialFact?.title ?? '');
  const [value, setValue] = useState(initialFact?.value ?? '');
  const [icon, setIcon] = useState<EpkFactIcon>(initialFact?.icon ?? DEFAULT_EPK_FACT_ICON);
  return (
    <FormDialog title={isEditing ? 'Modifier l’élément' : 'Ajouter un élément'} closeDisabled={saving} onClose={onClose}>
      <form className="space-y-4" onSubmit={(event) => {
        event.preventDefault();
        if (!value.trim()) return;
        onAdd({ id: initialFact?.id ?? `fact-${Date.now()}`, title: title.trim(), value: value.trim(), icon });
        onClose();
      }}>
        <Field id="new-fact-title" label="Titre" value={title} optional onChange={setTitle} />
        <Field id="new-fact-value" label="Valeur" value={value} required onChange={setValue} />
        <FactIconPicker id="epk-new-fact-icon" value={icon} onChange={setIcon} />
        <Button type="submit" variant="primary" fullWidth loading={saving} disabled={!value.trim()}>{isEditing ? 'Enregistrer les modifications' : 'Ajouter l’élément'}</Button>
      </form>
    </FormDialog>
  );
}
...
export function DocumentFormDialog({ saving, onClose, onUpload, onUpdate, initialDocument }: { saving: boolean; onClose: () => void; onUpload?: (file: File, title: string, description: string, icon: EpkDocumentIcon) => void; onUpdate?: (title: string, description: string, icon: EpkDocumentIcon, file?: File) => void; initialDocument?: EpkDocument | null }) {
  ...
  const [icon, setIcon] = useState<EpkDocumentIcon>(initialDocument?.icon ?? DEFAULT_EPK_DOCUMENT_ICON);
  const canSubmit = Boolean(title.trim() && (isEditing || file));
  return (
    <FormDialog title={isEditing ? 'Modifier le fichier' : 'Ajouter un fichier'} closeDisabled={saving} onClose={onClose}>
      <form className="space-y-4" onSubmit={(event) => {
        event.preventDefault();
        if (!title.trim()) return;
        if (isEditing) {
          onUpdate?.(title.trim(), description.trim(), icon, file ?? undefined);
        } else {
          if (!file) return;
          onUpload?.(file, title.trim(), description.trim(), icon);
        }
        onClose();
      }}>
        <Field id="new-document-title" label="Nom" value={title} required onChange={setTitle} />
        <div>
          <FieldLabel htmlFor="epk-new-document-description" optional>Description</FieldLabel>
          <TextArea id="epk-new-document-description" rows={3} value={description} onChange={(event) => setDescription(event.target.value)} />
        </div>
        <DocumentIconPicker id="epk-new-document-icon" value={icon} onChange={setIcon} />
        <FieldLabel as="span" required={!isEditing} optional={isEditing}>Fichier</FieldLabel>
        <input ref={inputRef} className="sr-only" aria-label="Fichier PDF ou ZIP" type="file" accept=".pdf,.zip,application/pdf,application/zip,application/x-zip-compressed" disabled={saving} onChange={(event) => setFile(event.target.files?.[0] ?? null)} />
        <Button variant="secondary" fullWidth disabled={saving} onClick={() => inputRef.current?.click()}>{file ? file.name : isEditing ? 'Remplacer le fichier' : 'Choisir un fichier'}</Button>
        <p className="text-xs text-[var(--fz-text-muted)]">PDF ou ZIP · 15 Mo maximum.</p>
        <Button type="submit" variant="primary" fullWidth loading={saving} disabled={!canSubmit}>{isEditing ? 'Enregistrer les modifications' : 'Ajouter le fichier'}</Button>
      </form>
    </FormDialog>
  );
}
export function FactIconPicker({ id, value, onChange }: { id: string; value: EpkFactIcon; onChange: (value: EpkFactIcon) => void }) {
  return <IconMenuPicker id={id} value={value} onChange={onChange} options={factIcons} usagePrefix="epk.fact-picker" />;
}
export function DocumentIconPicker({ id, value, onChange }: { id: string; value: EpkDocumentIcon; onChange: (value: EpkDocumentIcon) => void }) {
  return <IconMenuPicker id={id} value={value} onChange={onChange} options={documentIcons} usagePrefix="epk.document-picker" />;
}
function IconMenuPicker<T extends string>({ id, value, onChange, options, usagePrefix }: { id: string; value: T; onChange: (value: T) => void; options: Array<{ value: T; label: string }>; usagePrefix: string }) {
  ...
  return (
    <div ref={rootRef}>
      <FieldLabel htmlFor={id}>Icône</FieldLabel>
      <button
        id={id}
        type="button"
        role="combobox"
        ...
        className="fz-text-field mt-2 flex items-center gap-3 text-left"
      >
        <FzIcon name={selected.value} usageId={`${usagePrefix}.selected.${selected.value}`} size="lg" />
        <span className="min-w-0 flex-1">{selected.label}</span>
        <FzIcon name="chevron-down" usageId={`${usagePrefix}.chevron`} size="sm" />
      </button>
      {open ? (
        <ul id={listId} role="listbox" aria-label="Icône" className="mt-2 max-h-56 overflow-y-auto rounded-2xl border border-white/10 bg-[#18181b] p-1">
          {options.map((option) => {
            ...
            return (
              <li key={option.value}>
                <button
                  ...
                  className={`flex min-h-12 w-full items-center gap-3 rounded-xl px-3 text-left text-base transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-rose-300 ${isSelected ? 'bg-[color:var(--fz-accent)]/15 text-[var(--fz-accent)]' : 'text-white hover:bg-white/8'}`}
                >
                  <FzIcon name={option.value} usageId={`${usagePrefix}.${option.value}`} size="lg" />
                  {option.label}
                </button>
              </li>
            );
...
function Card({ children, onRemove, onEdit, disabled }: { children: ReactNode; onRemove: () => void; onEdit?: (() => void) | undefined; disabled: boolean }) { return <div className="flex items-center gap-3 rounded-xl border border-white/10 p-3"><div className="min-w-0 flex-1 space-y-1">{children}</div><div className="flex items-center gap-1.5 shrink-0">{onEdit ? <EditIconButton label="Modifier cet élément" usageId="epk.card.edit" disabled={disabled} onClick={onEdit} /> : null}<DeleteIconButton label="Supprimer cet élément" usageId="epk.collection.delete" disabled={disabled} onClick={onRemove} /></div></div>; }
