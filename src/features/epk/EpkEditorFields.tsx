...
type Props = {
  epk: EpkRecord; onChange: (next: EpkRecord) => void; onSave: () => void; onPublish: () => void; saving: boolean;
...
export function EpkEditorFields(props: Props) {
  ...
  const [addDialog, setAddDialog] = useState<'fact' | 'track' | 'video' | 'document' | null>(null); const [contactDialogOpen, setContactDialogOpen] = useState(false); const [editingContact, setEditingContact] = useState<EpkContact | null>(null);
  ...
  return <><div className="space-y-5">
    <div className="space-y-2"><Button variant="primary" fullWidth loading={saving} onClick={props.onSave}>Enregistrer les modifications</Button><Button variant="secondary" fullWidth loading={saving} onClick={props.onPublish}>{epk.status === 'PUBLISHED' ? 'Mettre à jour la page publique' : 'Publier la page'}</Button></div>
