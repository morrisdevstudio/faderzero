...
export function EpkPage() {
  ...
  async function setFeaturedVideo(video: EpkVideo) {
    if (!epk) return;
    setSaving(true); setMessage(null);
    try { const value = await saveEpk({ ...epk, featuredType: 'VIDEO', featuredId: video.id }); setEpk(value); setMessage('Média principal défini.'); }
    catch (error) { setMessage(error instanceof Error ? error.message : 'Impossible de définir le média principal.'); }
    finally { setSaving(false); }
  }
  ...
  return <div className="space-y-5 pb-6"><DetailHeader title="EPK public" subtitle={epk.status === 'PUBLISHED' ? 'Publié' : 'Brouillon'} onBack={() => navigate('/account?tab=groupe')} backLabel="Retour aux paramètres" />
    ...
    {firstVideo ? <section className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 space-y-3"><p className="text-sm text-white/75">Média principal</p><p className="text-xs text-white/55">Sélectionnez la vidéo qui sera utilisée pour rendre l’EPK publiable.</p><Button variant="secondary" fullWidth loading={saving} onClick={() => void setFeaturedVideo(firstVideo)}>{epk.featuredType === 'VIDEO' && epk.featuredId === firstVideo.id ? 'Vidéo principale sélectionnée' : `Définir « ${firstVideo.title || firstVideo.provider} » comme média principal`}</Button></section> : null}
