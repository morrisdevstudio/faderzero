...
function Section({ section, model, visible, editing, onEditSection }: { section: EpkSectionId; model: EpkPublicModel; visible: boolean; editing: boolean; onEditSection?: (section: EpkSectionId) => void }) {
  ...
  if (section === 'banniere') {
    ...
    const hasMusic = model.tracks.length > 0 || model.links.some((link) => { const brand = brandForLink(link); return normalizeBrandLabel(link.label) !== 'twitch' && (!brand || !socialBrands.has(brand)); });
    const hasProDocuments = model.documents.length > 0;
    return <section id="banniere" className={heroUrl ? 'epk-hero' : 'epk-hero epk-hero-no-image'} style={heroUrl ? { backgroundImage: `url(${heroUrl})` } : undefined}><div className="epk-hero-shade" /><div className="epk-hero-content"><span className="epk-chip">{model.genres.join(' · ') || 'Electronic press kit'}</span><h1>{model.name}</h1>{model.tagline ? <p>{model.tagline}</p> : null}{hasMusic || hasProDocuments ? <div className="epk-actions">{hasMusic ? <a href="#musique"><FzIcon name="play" usageId="epk.hero.listen" />Écouter</a> : null}{hasProDocuments ? <a href="#espace-pro"><FzIcon name="folder" usageId="epk.hero.pro" />Espace Pro</a> : null}</div> : null}</div></section>;
  }
  if (section === 'bio') return <section id="bio" className="epk-section epk-bio"><div><h2>{model.editorial.bioTitle}</h2><p className="epk-prose">{model.fullBio || model.shortBio}</p></div>{model.editorial.facts.length ? <aside><h3>En bref</h3>{model.editorial.facts.map((fact) => <div className="epk-fact" key={fact.id}><span><FzIcon name={normalizedEpkFactIcon(fact.icon)} usageId={`epk.fact.${fact.icon}`} /></span><p><small>{fact.title}</small>{fact.value}</p></div>)}</aside> : null}</section>;
  if (section === 'musique') { const platformLinks = model.links.filter((link) => { const brand = brandForLink(link); return normalizeBrandLabel(link.label) !== 'twitch' && (!brand || !socialBrands.has(brand)); }); return <section id="musique" className="epk-section epk-music"><header><h2>{model.editorial.musicTitle}</h2></header><div className={model.tracks.length ? 'epk-music-grid' : 'epk-music-grid epk-music-grid-platforms-only'}>{model.tracks.length ? <EpkAudioPlayer model={model} /> : null}<div className="epk-platforms">{platformLinks.map((link) => { const brand = brandForLink(link); return <a href={link.url} target="_blank" rel="noreferrer" key={`${link.label}-${link.url}`}><span>{brand ? <BrandIcon name={brand} /> : <FzIcon name="music" usageId="epk.platform.icon" />}</span><p><strong>{link.label}</strong><small>Écouter sur la plateforme</small></p><FzIcon name="external-link" usageId="epk.platform.external" size="sm" /></a>; })}</div></div></section>; }
  if (section === 'medias') return <section id="medias" className="epk-section epk-media"><header><h2>{model.editorial.mediaTitle}</h2></header>{model.videos.length ? <div className={model.videos.length === 1 ? 'epk-videos epk-videos-one' : 'epk-videos'}>{model.videos.map((video) => <EpkVideoEmbed key={video.id} video={video} />)}</div> : null}<PhotoCarousel photos={model.photos} name={model.name} /></section>;
