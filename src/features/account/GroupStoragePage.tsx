  ...
  return (
    <section aria-label="Stockage du groupe" className="space-y-5">
        ...
        <article key={connection.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-white/55">{statusLabels[connection.status]}{connection.isDefault ? ' · nouveaux envois' : ''}</p>
            ...
            <span className="rounded-full bg-emerald-400/10 px-2 py-1 text-xs font-bold text-emerald-200">{statusLabels[connection.status]}</span>
          </div>
      ...
      <article className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
        ...
        <Button className="mt-4" disabled={busyConnectionId !== null} onClick={() => void connectGoogleDrive()}>{connections.some((item) => item.providerId === 'google_drive') ? 'Reconnecter Google Drive' : 'Connecter Google Drive'}</Button>
      </article>
      {connections.filter((item) => item.status === 'connected').length > 1 ? (
        <article className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          ...
          {transferReport ? <p className="mt-3 text-sm text-emerald-200">{transferReport.copied} fichier(s) transféré(s), {transferReport.failed.length} échec(s). Les échecs peuvent être relancés.</p> : null}
