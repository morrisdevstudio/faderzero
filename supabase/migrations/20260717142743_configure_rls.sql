-- =====================================================================
-- 02_RLS.SQL
-- Activation de RLS, fonctions utilitaires sÃ©curisÃ©es et policies.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- FONCTION SECURISÃ‰E POUR EVITER LA RECURSION SUR WORKSPACE_MEMBERS
-- ---------------------------------------------------------------------
...
-- RÃ©duire les privilÃ¨ges d'exÃ©cution au strict minimum
...
-- ---------------------------------------------------------------------
CREATE POLICY "Les profils sont visibles par tous les utilisateurs connectÃ©s"
    ON public.profiles FOR SELECT
    TO authenticated
    USING (true);

CREATE POLICY "Les utilisateurs peuvent insÃ©rer leur propre profil"
...
CREATE POLICY "Tout utilisateur connectÃ© peut crÃ©er un workspace"
...
CREATE POLICY "Le crÃ©ateur du workspace ou un membre existant peut ajouter des membres"
...
CREATE POLICY "Les membres peuvent Ãªtre modifiÃ©s par le groupe"
    ON public.workspace_members FOR UPDATE
    TO authenticated
    USING (public.check_is_workspace_member(workspace_id))
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les membres peuvent Ãªtre supprimÃ©s par le groupe"
...
CREATE POLICY "Les invitations peuvent Ãªtre crÃ©Ã©es par les membres"
    ON public.workspace_invites FOR INSERT
    TO authenticated
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les invitations peuvent Ãªtre mises Ã  jour par les membres"
    ON public.workspace_invites FOR UPDATE
    TO authenticated
    USING (public.check_is_workspace_member(workspace_id))
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les invitations peuvent Ãªtre supprimÃ©es par les membres"
...
CREATE POLICY "Les morceaux peuvent Ãªtre crÃ©Ã©s par les membres du workspace"
    ON public.songs FOR INSERT
    TO authenticated
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les morceaux peuvent Ãªtre modifiÃ©s par les membres du workspace"
    ON public.songs FOR UPDATE
    TO authenticated
    USING (public.check_is_workspace_member(workspace_id))
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les morceaux peuvent Ãªtre supprimÃ©s par les membres du workspace"
...
CREATE POLICY "Les setlists peuvent Ãªtre crÃ©Ã©es par les membres du workspace"
    ON public.setlists FOR INSERT
    TO authenticated
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les setlists peuvent Ãªtre modifiÃ©es par les membres du workspace"
    ON public.setlists FOR UPDATE
    TO authenticated
    USING (public.check_is_workspace_member(workspace_id))
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les setlists peuvent Ãªtre supprimÃ©es par les membres du workspace"
...
CREATE POLICY "Les liaisons setlist_songs peuvent Ãªtre crÃ©Ã©es par les membres du workspace"
    ON public.setlist_songs FOR INSERT
    TO authenticated
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les liaisons setlist_songs peuvent Ãªtre modifiÃ©es par les membres"
    ON public.setlist_songs FOR UPDATE
    TO authenticated
    USING (public.check_is_workspace_member(workspace_id))
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les liaisons setlist_songs peuvent Ãªtre supprimÃ©es par les membres"
...
CREATE POLICY "Les assets song_assets peuvent Ãªtre crÃ©Ã©s par les membres du workspace"
    ON public.song_assets FOR INSERT
    TO authenticated
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les assets song_assets peuvent Ãªtre modifiÃ©s par les membres"
    ON public.song_assets FOR UPDATE
    TO authenticated
    USING (public.check_is_workspace_member(workspace_id))
    WITH CHECK (public.check_is_workspace_member(workspace_id));

CREATE POLICY "Les assets song_assets peuvent Ãªtre supprimÃ©s par les membres"
