-- EPK editor: a mutable private draft and an immutable public snapshot.
ALTER TABLE public.epks
  ADD COLUMN IF NOT EXISTS accent_color text NOT NULL DEFAULT '#ff3a63' CHECK (accent_color ~ '^#[0-9A-Fa-f]{6}$'),
  ADD COLUMN IF NOT EXISTS section_order text[] NOT NULL DEFAULT ARRAY['presentation','videos','audio','photos','documents','contacts','links'],
...
CREATE OR REPLACE FUNCTION private.epk_sections_are_unique(p_sections text[])
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path = pg_catalog AS $$
  SELECT cardinality(p_sections) = cardinality(ARRAY(SELECT DISTINCT unnest(p_sections)));
$$;
REVOKE ALL ON FUNCTION private.epk_sections_are_unique(text[]) FROM PUBLIC;
...
ALTER TABLE public.epks ADD CONSTRAINT epks_section_order_valid CHECK (
  section_order <@ ARRAY['presentation','videos','audio','photos','documents','contacts','links']
...
ALTER TABLE public.epks ADD CONSTRAINT epks_hidden_sections_valid CHECK (
  hidden_sections <@ ARRAY['presentation','videos','audio','photos','documents','contacts','links']
  AND NOT ('contacts' = ANY(hidden_sections))
);
...
BEGIN
  ...
  SELECT jsonb_build_object('version',1,'revision',v_epk.draft_revision,'publishedAt',now(),'name',v_epk.display_name,'slug',v_epk.slug,'tagline',v_epk.tagline,'shortBio',v_epk.short_bio,'fullBio',v_epk.full_bio,'city',v_epk.city,'country',v_epk.country,'genres',to_jsonb(v_epk.genres),'accentColor',v_epk.accent_color,'sectionOrder',to_jsonb(v_epk.section_order),'hiddenSections',to_jsonb(v_epk.hidden_sections), 'videos',(SELECT coalesce(jsonb_agg(jsonb_build_object('id',id,'title',title,'provider',provider,'providerVideoId',provider_video_id) ORDER BY position),'[]') FROM public.epk_videos WHERE epk_id=v_epk.id), 'tracks',(SELECT coalesce(jsonb_agg(jsonb_build_object('id',id,'title',title,'description',description) ORDER BY position) FILTER (WHERE visibility='PUBLIC'),'[]') FROM public.epk_tracks WHERE epk_id=v_epk.id), 'photos',(SELECT coalesce(jsonb_agg(jsonb_build_object('id',id,'previewAssetId',preview_asset_id,'caption',caption,'credit',credit) ORDER BY position),'[]') FROM public.epk_photos WHERE epk_id=v_epk.id), 'documents',(SELECT coalesce(jsonb_agg(jsonb_build_object('id',id,'assetId',asset_id,'title',title,'type',document_type,'updatedAt',document_updated_at) ORDER BY position),'[]') FROM public.epk_documents WHERE epk_id=v_epk.id), 'contacts',(SELECT coalesce(jsonb_agg(jsonb_build_object('name',name,'role',role,'email',email,'phone',phone,'whatsapp',whatsapp) ORDER BY position),'[]') FROM public.epk_contacts WHERE epk_id=v_epk.id), 'links',(SELECT coalesce(jsonb_agg(jsonb_build_object('label',coalesce(label,kind),'url',url) ORDER BY position),'[]') FROM public.epk_links WHERE epk_id=v_epk.id)) INTO v_snapshot;
