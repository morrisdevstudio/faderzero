...
Routes migrées : `/home`, `/songs`, `/setlists`, `/calendar`, `/metronome` et `/prompter`. `/booking` reste une sous-page hors de ce périmètre.
...
## Composants validés en cours de migration

- `AddButton`, `PageHeader`, `DetailHeader`, `AppHeader` et `StatusPill` sont les noms canoniques validés.
- Les cinq composants disposent maintenant d’une implémentation canonique locale ; leur déploiement sur les écrans reste progressif.
